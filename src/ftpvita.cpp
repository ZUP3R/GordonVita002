...
static void cmd_PASV_func(ftpvita_client_info_t *client)
{
	...
	my_sprintf(data_socket_name, "FTPVita_client_%i_data_socket",
		client->num);
	...
	/* Build the command */
	my_sprintf(cmd, "227 Entering Passive Mode (%hhu,%hhu,%hhu,%hhu,%hhu,%hhu)" FTPVITA_EOL,
...
static int gen_list_format(char *out, int n, int dir, const SceIoStat *stat, const char *filename)
{
	...
	if  (cdt.year == stat->st_mtime.year) {
		my_snprintf(yt, sizeof(yt), "%02d:%02d", stat->st_mtime.hour, stat->st_mtime.minute);
	}
	else {
		my_snprintf(yt, sizeof(yt), "%04d", stat->st_mtime.year);
	}

	return my_snprintf(out, n,
...
static void send_LIST(ftpvita_client_info_t *client, const char *path)
{
	...
	if (send_devices) {
		...
		my_memset(&dirent, 0, sizeof(dirent));
...
{
	char msg[PATH_MAX];
	my_snprintf(msg, sizeof(msg), "257 \"%s\" is the current directory." FTPVITA_EOL, client->cur_path);
...
static void cmd_CWD_func(ftpvita_client_info_t *client)
{
	...
	if (n < 1) {
		...
		if (my_strcmp(cmd_path, "/") == 0) {
			my_strcpy(client->cur_path, cmd_path);
		} else  if (my_strcmp(cmd_path, "..") == 0) {
			dir_up(client->cur_path);
		} else {
			if (cmd_path[0] == '/') { /* Full path */
				my_strcpy(tmp_path, cmd_path);
				...
				if (path_is_at_root(client->cur_path))
					my_snprintf(tmp_path, sizeof(tmp_path), "%s%s", client->cur_path, cmd_path);
				else
					my_snprintf(tmp_path, sizeof(tmp_path), "%s/%s", client->cur_path, cmd_path);
			}

			/* If the path is like: /foo: add an slash */
			if (my_strrchr(tmp_path, '/') == tmp_path)
				my_strcat(tmp_path, "/");

			/* If the path is not "/", check if it exists */
			if (my_strcmp(tmp_path, "/") != 0) {
			...
			my_strcpy(client->cur_path, tmp_path);
		}
...
static void gen_ftp_fullpath(ftpvita_client_info_t *client, char *path, size_t path_size)
{
	...
	if (cmd_path[0] == '/') {
		/* Full path */
		my_strncpy(path, cmd_path, path_size);
	} else {
		if (my_strlen(cmd_path) >= 5 && cmd_path[3] == ':' && cmd_path[4] == '/') {
			/* Case "ux0:/foo */
			my_snprintf(path, path_size, "/%s", cmd_path);
		} else {
			/* The file is relative to current dir, so
			 * append the file to the current path */
			my_snprintf(path, path_size, "%s/%s", client->cur_path, cmd_path);
		}
	}
}
...
static void cmd_RNFR_func(ftpvita_client_info_t *client)
{
	...
	my_strcpy(client->rename_path, vita_path_src);
...
static void cmd_SIZE_func(ftpvita_client_info_t *client)
{
	...
	my_sprintf(cmd, "213 %lld" FTPVITA_EOL, stat.st_size);
	client_send_ctrl_msg(client, cmd);
}
...
static void cmd_REST_func(ftpvita_client_info_t *client)
{
	...
	my_sprintf(cmd, "350 Resuming at %d" FTPVITA_EOL, client->restore_point);
	client_send_ctrl_msg(client, cmd);
}
...
static cmd_dispatch_func get_dispatch_func(const char *cmd)
{
	...
	for(i = 0; cmd_dispatch_table[i].cmd && cmd_dispatch_table[i].func; i++) {
		if (my_strcmp(cmd, cmd_dispatch_table[i].cmd) == 0) {
	...
	for(i = 0; i < MAX_CUSTOM_COMMANDS; i++) {
		if (custom_command_dispatchers[i].valid) {
			if (my_strcmp(cmd, custom_command_dispatchers[i].cmd) == 0) {
...
static int client_thread(SceSize args, void *argp)
{
	...
	while (1) {
		my_memset(client->recv_buffer, 0, sizeof(client->recv_buffer));
		...
		if (client->n_recv > 0) {
			...
			client->recv_cmd_args = my_strstr(client->recv_buffer, " ");
...
static int server_thread(SceSize args, void *argp)
{
	...
	while (1) {
		...
		if (client_sockfd >= 0) {
			...
			my_sprintf(client_thread_name, "FTPVita_client_%i_thread",
				number_clients);
			...
			client->data_con_type = FTP_DATA_CONNECTION_NONE;
			my_strcpy(client->cur_path, FTP_DEFAULT_PATH);
...
int ftpvita_init(char *vita_ip, unsigned short int *vita_port)
{
	...
	/* Return data */
	my_strcpy(vita_ip, info.ip_address);
	*vita_port = FTP_PORT;
...
int ftpvita_add_device(const char *devname)
{
	...
	for (i = 0; i < MAX_DEVICES; i++) {
		if (!device_list[i].valid) {
			my_strcpy(device_list[i].name, devname);
...
int ftpvita_del_device(const char *devname)
{
	int i;
	for (i = 0; i < MAX_DEVICES; i++) {
		if (my_strcmp(devname, device_list[i].name) == 0) {
			device_list[i].valid = 0;
			return 1;
		}
	}
